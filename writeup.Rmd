...
In this assignment the modeling effort consisted of data preparation, selecting factors, partitioning the training data, building the model, and evaluating the result. The resulting model uses a random forest algorithm trained against a partitioned data set with a 75%/25% split for traing/validation. Its out of sample error is something

##data preparation


The training set, at nearly 20k rows, seems large enough to comfortably create a cross validation set. I chose random subsampling without replacement to evenly distribute test data across all participants

Data cleanup aims to 
...
I used a random forest to develop the model for its strength in classification and mitigation of bias and overfitting, which is useful for developing a generalized model with only 6 training subjects. In another alternative attempt using _generalized linear model_ algorithm, the algorithm was unable to establish predictors from the dataset.

#Training

I ran a prediction. The random forest model yielded 99.98% accuracty with 28 parameters chosen from among 55 factors on a set of 14718 samples. Bootstrapping was done with 25 reps and sample sizes of 14718
...
print(result3$model$finalModel)
```


#Out of Sample Error

The random forest algorithm averages the out of bag error at each branch, and yields its own estimate of out of sample error from the training set, estimated to be 0.09%

```{r finalModel, echo=FALSE}
result3$model$finalModel
```

On a separate, conventional validation, the model misclassfied a single sample and had a 99.98% accuracy rate on the testing data held out of the training set. 

```{r matrix, echo=FALSE}
result3$confusionMatrix
```

The predictive model run against the testing set yields the following predictions

```{r prediction, echo=FALSE}
prediction <- predict(result3$model, dataTestingPml)
print(prediction)
```

##conclusions

The random forest model has shown to be predict accurately against the validation set and through cross validation. The test set draws from the same set of data, and I expect it will predict accurately against it. However, the model is tuned to a sample from 6 select subjects and its unclear how well this model will generalize to other subjects.
